var encodeURL,show_animation,hide_animation,apply,apply_multiple,apply_none,apply_img,apply_any,apply_video,apply_link,apply_file_rename,apply_file_duplicate,apply_folder_rename;
(function ($, Modernizr, image_editor)
{
		...
		contextActions: {
			...
			edit_img: function($trigger)
			{
				...
				if($('#ftp').val()==true){
					...
					var full_path = $('#static_url').val() + filename;
				}
			...
			grid.on('click', '.rename-file', function ()
			{
				...
				bootbox.prompt($('#rename').val(), $('#cancel').val(), $('#ok').val(), function (name)
				{
					if (name !== null)
					{
						name = fix_filename(name);
						if(_check_thai(name)){
							alert('ชื่อไฟล์ไม่สามารถใช้ภาษาไทยได้');
						}
						else{
							if (name != old_name)
							{
	...
	{
		var tmp = url.split('/');
		var static_url = window.location.href.split('tinymce')[0] + $('#cur_dir').val().split('/images/')[1];
		url = static_url + tmp[tmp.length - 1];
...
// because of feedback and on error bahhhhh...
	...
	apply = function(file, external)
	{
		...
		static_url = window.location.href.split('tinymce')[0] + $('#cur_dir').val().split('/images/')[1];
		var is_nopath_file = $('#is_nopath_file').val();
		var alt_name = file.substr(0, file.lastIndexOf('.'));
		var ext = file.split('.').pop();
		ext = ext.toLowerCase();
		
		var url = encodeURL(static_url + file);

		var target = $('#' + external, windowParent.document);
		target.val(url).trigger('change');
		if (typeof windowParent['responsive_filemanager_callback_' + external] == 'function')
		{
			windowParent['responsive_filemanager_callback_' + external].apply(null, [[url], [ext], [alt_name], external]);
		}
		close_window();
	}
	
	apply_multiple = function(files, external)
	{
		var windowParent;
		...
		if (typeof windowParent['responsive_filemanager_callback_' + external] == 'function')
		{
			windowParent['responsive_filemanager_callback_' + external].apply(null, [arr_url, arr_ext, arr_name, external]);
		}
		close_window();
	}
	...
	apply_link = function(file, external)
	{
		...
		if($('#ftp').val()==true){
			...
			if(is_nopath_file == 'Y'){
				...
				url = encodeURL((is_return_relative_url == 1 ? subdir : static_url) + file);
			}
		}
