...
<html xmlns="https://www.w3.org/1999/xhtml">
		...
		<?php
			...
			onSave: function(imageID, newURL) {
				...
				$.ajax({
					...
					data: { url: newURL, path:$('#sub_folder').val()+$('#fldr_value').val(), name:$('#aviary_img').attr('data-name'), static_url:$('#static_url').val() }
				}).done(function( msg ) {
					featherEditor.close();
					d = new Date();
	...
	<script src="js/include.js?<?php echo time(); ?>"></script>
	<style>
		.list-view0 input[type='checkbox']{
			position: absolute; margin: -120px 5px 5px 105px;
			z-index: 1;
		...
		.list-view1 input[type='checkbox']{
			position: absolute; margin: -25px 5px 5px 445px;
			z-index: 1;
			/* Double-sized Checkboxes */
			-ms-transform: scale(1.5); /* IE */
			-moz-transform: scale(1.5); /* FF */
		...
		.list-view2 input[type='checkbox']{
			position: absolute; margin: -25px 5px 5px 265px;
			z-index: 1;
			/* Double-sized Checkboxes */
			-ms-transform: scale(1.5); /* IE */
			-moz-transform: scale(1.5); /* FF */
...
<body>
	...
	<input type="hidden" id="static_url" value="<?php echo $static_url;?>" />
	<input type="hidden" id="is_nopath_file" value="<?php echo $is_nopath_file;?>" />
<?php if($upload_files){ ?>
<!-- uploader div start -->
<div class="uploader">
		...
		<div class="tab-content">
			<div class="tab-pane active" id="tab1">
				...
				<form action="dialog.php" method="post" enctype="multipart/form-data" id="rfmDropzone" class="dropzone">
					...
					<p style="color:#dd4b39;text-align:center;margin-top: 100px;">
						ขนาดไฟล์ไม่เกิน 16 MB / 1 ไฟล์
						<br />
						jpg, png, gif, doc, docx, pdf, xls, xlsx, ppt, pttx
					</p>
					<div class="fallback">
	...
	<div class="span12">
		...
		foreach ($files as $file_array) {
			...
			$file=$file_array['file'];
				...
				$mini_src = str_replace($current_path,$static_url,$mini_src);
				$src_thumb = str_replace($current_path,$static_url,$src_thumb);
				//$src_preview = str_replace($current_path,$static_url,str_replace($base_url,'',$src));
				
				$tmp_preview = explode('/images/', $base_url.$_SERVER['REQUEST_URI']);
				$tmp_preview_img = explode('/images/', str_replace($current_path,$static_url,str_replace($base_url,'',$src)));
				$src_preview = $tmp_preview[0].'/images/'.$tmp_preview_img[1];
...
?>
			...
			<figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				...
				<figcaption>
					...
					<a class="tip-right preview" title="<?php echo trans('Preview')?>" data-url="<?php echo $src_preview;?>" data-toggle="lightbox" href="#previewLightbox"><i class=" icon-eye-open"></i></a>
			...
			<?php if(@$multi == true && $is_img){ ?>
			<input type="checkbox" class="selected_imgs" value="<?php echo $file;?>" />
			<?php } ?>
		</li>
			<?php
			}
	...
	?></div>
		...
		<?php if(@$multi == true){ ?>
		<!--
		<button type="button" onclick="delete_select_imgs();" style="float: right; margin-top: 20px; margin-bottom: 50px; height: 30px; background-color: red;"><span class="icon-trash"></span> Delete Select Images</button>
		-->
		<button type="button" onclick="apply_select_imgs();" style="float: right; margin-top: 20px; margin-bottom: 50px; height: 30px; background-color: green; margin-right: 10px;"><span class="icon-plus"></span> Insert Select Images</button>
		<button type="button" onclick="select_all_imgs();" style="float: right; margin-top: 20px; margin-bottom: 50px; height: 30px; background-color: grey; margin-right: 10px;"><span class="icon-check"></span> Select All</button>
		<?php } ?>
		<?php } ?>
	</div>
	</div>
...
<script>
	...
	function apply_select_imgs()
	{
		var _imgs = [];
		$('.selected_imgs:checked').each(function () {
			_imgs.push($(this).val());
		});
		apply_multiple(_imgs.join("||"), '<?php echo $_GET['field_id']; ?>');
	}
	
	function select_all_imgs()
	{
		$('.selected_imgs').prop('checked', true);
	}
	
	function delete_select_imgs()
	{
		if(confirm('คุณแน่ใจว่าต้องการลบรูปภาพที่เลือก ?'))
		{
			var _imgs = [];
			var _imgs2 = '';
			$('.selected_imgs:checked').each(function () {
			...
			var request = $.ajax({
				type: "POST",
				url: "execute.php?action=delete_files",
				data: { path: _imgs2, paths: _imgs.join("||") }
			});
	...
	<?php if (is_array($files_prevent_duplicate) || is_object($files_prevent_duplicate)) : ?>
