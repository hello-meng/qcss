...
if ( ! empty($_FILES))
{
	...
	if (in_array(fix_strtolower($extension), $ext))
	{
		...
		$_FILES['file']['name'] = mb_substr(str_replace(".".fix_strtolower($extension), "", fix_strtolower($_FILES['file']['name'])), 0, 29).'.'.fix_strtolower($extension);
		...
		// Replace Text
		$find_txt  = array(' ', '%', '#', '<', '$', '+', '>', '!', '`', '&', '*', '‘', '|', '{', '?', '“', '=', '}', '/', ':', '\\', '@');
		$_FILES['file']['name'] = str_replace($find_txt,'',$_FILES['file']['name']);

		// Check Thai Language 
		if (preg_match("/[ก-๙]+/", $_FILES['file']['name'])) {
			$_FILES['file']['name'] = rand(100000,999999).".".$extension;
		}
		...
		$targetFileThumb2 =  $targetPathThumb.'square-'.$_FILES['file']['name'];
		...
		if ($is_img)
		{
			...
			if ( ! create_img($targetFile, $targetFileThumb, 220, 148) || ! create_img($targetFile, $targetFileThumb2, 130, 130))
			{
				$memory_error = FALSE;
			}
			else
