...
class province_model extends CI_Model {
    ...
    function count_all()
    {
        ...
        if($fName != "")
        {
            $this->db->where("(province.name_th LIKE '%".$fName."%' or province.name_en LIKE '%".$fName."%')");
        }

        $fStatus = @$this->input->post('columns')[2]['search']['value'];
        if($fStatus != "")
        {
            $this->db->where('province.status', $fStatus);
        }

        $this->db->where("province.status <> 'D'");
    ...
    {
        $this->db->select('province.*, region.name_th as region');
        $this->db->from('province');
