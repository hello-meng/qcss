...
class section_model extends CI_Model {
    ...
    function count_all()
    {
        ...
        if($fName != "")
        {
            $this->db->where("(backend_section.name LIKE '%".$fName."%')");
        }

        $fStatus = @$this->input->post('columns')[1]['search']['value'];
        if($fStatus != "")
        {
            $this->db->where('backend_section.status', $fStatus);
        }

        $this->db->where("backend_section.status <> 'D'");
    ...
    {
        $this->db->select('backend_section.*');
        $this->db->from('backend_section');
    ...
    function insert()
    {
        ...
        $data['updated_by'] = '-';
        $this->db->insert('backend_section', $data);
    }
    ...
    function update()
    {
        ...
        $this->db->where('id', $this->uri->segment(4));
        $this->db->where("status <> 'D'");
        $this->db->update('backend_section', $data);
    }
