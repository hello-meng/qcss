...
class bank_model extends CI_Model {
    ...
    function count_all()
    {
        ...
        if($fCode != "")
        {
            $this->db->where('bank.code', $fCode);
        }
        
        $fName = @$this->input->post('columns')[1]['search']['value'];
        if($fName != "")
        {
            $this->db->where("(bank.name_th LIKE '%".$fName."%' or bank.name_en LIKE '%".$fName."%')");
        }

        $fStatus = @$this->input->post('columns')[2]['search']['value'];
        if($fStatus != "")
        {
            $this->db->where('bank.status', $fStatus);
        }

        $this->db->where("bank.status <> 'D'");
